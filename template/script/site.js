						...
						<div class="collapse navbar-collapse">
							<ul class="navbar-nav mr-auto">
								...
								<li class="nav-link pr-0">
									<span class="btn my-sm-0 bg-light text-dark"><del>${obj.cost}&#1423;</del></span>
								</li>
								<li class="nav-link pl-0">
									<span class="btn btn-outline-success my-sm-0 bg-secondary text-light">${obj.cost * (1-obj.discount/100)}&#1423;</span>
								</li>
							</ul>
						</div>
					</nav>
...
function getRecomendedItem(obj){
	...
	let recomendProducts = `
		...
		<div class="col-4 card card-comment">
			<img class="card-img" src="/template/images/${obj.id}.jpg" alt="Card image cap">
			${getSaleStatus(obj.discount)}
			
			<div class="card-body text-center">
				<a href="/product/view/${obj.id}"><h5  class="card-title">${obj.descr}</h5></a>
				<span class=" mx-auto">
					${new Date(+obj.time_add).getFullYear()}-${month}-${date}
				</span>
				<p class="card-text">${obj['main']}</p>
	...
	<nav class=" navbar-expand-lg navbar-light input-group-append home-inputs">
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav mr-auto">
					...
					<li class="nav-link pr-0">
						<span class="btn  my-2 my-sm-0 bg-light text-dark item-cost">${obj.cost}</span>
					</li>
					<li class="nav-link pl-0">
						<span class="btn btn-outline-success my-2 my-sm-0 bg-secondary text-light item-cost">${obj.new_cost}</span>
					</li>
				</ul>
			</div>
		</nav>
